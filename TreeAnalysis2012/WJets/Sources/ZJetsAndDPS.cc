...
//        }
 ...
 if (DEBUG) cout << "Stop after line " << __LINE__ << endl;
    ...
    //unsigned int nEventsWithTwoGoodLeptonsNoChargeNoMass(0); 
    //unsigned int nEventsWithTwoGoodLeptonsNoMass(0); 
    ...
    //andrew debug
    int npassesGenLepSelector(0);
    int npassesGenLepBarePrompt(0);
    int numGenPromptNeutrinos(0);
    int numGenPromptElectrons(0);
    int numGenPromptMuons(0);
    int npassesGenLeptonAnalysisCuts(0);
    int numHasAtLeastTwoGenLeptons(0);
    int npassesGenLeptonCut(0);
    int npassesMTCut(0);
    int npassesMETcut(0);

    int npassesDPSPartonCut(0);

    int npassesBasicGenJetCuts(0);
    int npassesAnalysisGenJetCuts(0);
    int npassesGenJetCut(0);
    ...
    cout << "--- begin: " << endl;
    cout << "hasRecoInfo: " << hasRecoInfo << endl;
    cout << "hasGenInfo: " << hasGenInfo << endl; 
    cout << "doQCD: " << doQCD << endl; 
    cout << "doSSign: " << doSSign << endl; 
    cout << "doInvMassCut: " << doInvMassCut << endl; 
    cout << "METcut: " << METcut << endl; 
    cout << "doBJets: " << doBJets << endl; 
    cout << "doPUStudy: " << doPUStudy << endl;
    ...
    //==========================================================================================================//
    cout << "\n--- Phase space cuts ---" << endl;
    cout << "Jet pt: " << jetPtCutMin <<" to " << jetPtCutMax << endl;
    cout << "Jet rapidity: " << jetEtaCutMin / 10.0 << " to " << jetEtaCutMax / 10.0 << endl;
    cout << "Z eta: " << ZEtaCutMin <<" to " << ZEtaCutMax << endl;
    cout << "MET cut: " << METcut << endl;
    cout << "MT cut: " << MTCut << endl;

    cout << "\n--- Other selections ---" <<endl;
    cout << "doQCD: " << doQCD << endl;
    cout << "doSS: " << doSSign << endl;
    cout << "Inv. mass cut (not actually utilized in analysis?): " << doInvMassCut << endl;
    cout << "Use B jets: " << doBJets << endl;
    cout << "Do PU study: " << doPUStudy << endl;
    cout << "doDR: " << doDR << endl;
    cout << "doFlat: " << doFlat << endl;
 ...
 if (DEBUG) cout << "Stop after line " << __LINE__ << endl;
   ...
   //for (Long64_t jentry(0); jentry < 500000; jentry++){
        ...
        //andrew -- calculating weight for use in filling histograms later
...
//            }
            ...
            //fetching neutrino ID for corresponding lepton
            if (doW) nuID = 14;
...
//                if (GLepBareSt->at(i) == 3 && abs(GLepBareId->at(i)) == LeptonID ) countTauS3--;
                if (lepSelector) npassesGenLepSelector++;
                ...
                double charge;
                if (abs(GLepBareId->at(i)) == 12 || abs(GLepBareId->at(i)) == 14 || abs(GLepBareId->at(i)) == 16) {
                    numGenPromptNeutrinos++;
                    charge = 0.;
                }
                else if (GLepBareId->at(i) > 0) charge = -1.;
                else charge = 1.;
       
                if (abs(GLepBareId->at(i)) == 11 || abs(GLepBareId->at(i)) == 13){
                    if (abs(GLepBareId->at(i)) == 11) numGenPromptElectrons++;
                    if (abs(GLepBareId->at(i)) == 13) numGenPromptMuons++;
                }
                
                //andrew -- setting the initial pt, eta, phi, E values for lepton here (before charged lepton dressing)
                    ...
                    && ( abs(GLepBareId->at(i)) == LeptonID || (charge == 0 && doW) ) )
                 {
                    ...
                    if( fabs(genLep.charge) > 0 ){
                        ...
                        for (unsigned short j(0); j < nTotGenPhotons; j++){
                            ...
                            //andrew -- fetch the kinematics of each photon in the event
                            ...
                            //andrew -- line below if the photons from FSR are next to the muon within a dR cone size of 0.1
                            if (deltaR(tmpGenPho.Phi(), tmpGenPho.Eta(), genLepNoFSR.phi, genLepNoFSR.eta) <= 0.1 && !used){
                                //if they are, add the pT, eta, phi, mass of each photon's TLorentzVector to the muon TLorentzVector 
                                tmpGenLep += tmpGenPho;
                                usedGenPho.push_back(j);
                            }
                        }
                        //now, re-assign the genLep kinematics to that for the dressed version of the lepton (for charged leptons)
                        genLep.pt = tmpGenLep.Pt();
                        genLep.eta = tmpGenLep.Eta();
                        genLep.phi = tmpGenLep.Phi();
                        genLep.energy = tmpGenLep.E();
                    }//end charged lepton dressing
                    ...
                    //andrew -- analysis cut on leptons (both charged and neutrinos) -- W boson
                    if (doW && ((fabs(genLep.charge) > 0 && genLep.pt >= 26 && fabs(genLep.eta) <= 2.4) || (fabs(genLep.charge) == 0 && genLep.pt >= METcut))){
                        genLeptons.push_back(genLep); 
                        npassesGenLeptonAnalysisCuts++;
                    }
                }
            } //end loop over nTotGenLeptons in the event
...
if (DEBUG) cout << "Stop after line " << __LINE__ << endl;
            ...
            if (nGenLeptons >= 2){
                numHasAtLeastTwoGenLeptons++;
                
                // sort leptons by descending pt
                // andrew -- so we then sort all of good gen leptons by their pT
                // and we will pick the two highest pT objects, of which we require belo win the MT cut
                // to be exactly one muon and one neutrino
                ...
                if (doW){
                    //double genMT;
                    //here we mandate that the two highest pT objects be exactly one muon and one neutrino
                    //both have passed their respective analysis cuts above
                    //note: genLepton1 is always picked as the muon, and genLepton2 the neutrino, done to calculate the MT
                    ...
                    //andrew -- so the MT requirement is also the 1muon-1neutrino requirement..
                    //if the first two sorted leptons don't have the charge properties of 1 mu and 1 nu, 
                    //then the genMT variable is set at a really low negative value
                    //so this should cut out events with more than one muon that makes its way into the genLeptons collection

                    else genMT = -99.0;
                   
                    if (genMT >= MTCut) npassesMTCut++;
                    if (genLepton2.pt >= METcut) npassesMETcut++;
                    if (genMT >= MTCut && genLepton2.pt >= METcut) {
			passesGenLeptonCut = 1;
			if (passesGenLeptonCut) npassesGenLeptonCut++;
		    }
                }
...
if (DEBUG) cout << "Stop after line " << __LINE__ << endl;
        ...
        //        Retrieving gen jets         //
        //====================================//
        //andrew -- why was passesGenJetCut initialized to 1 before? -- 4.4.18
        //bool passesGenJetCut(1);
        bool passesGenJetCut(0);  
        bool passesGenEWKJetPt(0), passesGenEWKJetFwdEta(0);
        ...
        if (hasGenInfo){
            ...
            for (unsigned short i(0); i < nTotGenJets; i++){
                ...
                for (unsigned short j(0); j < nGenLeptons; j++){ 
                    ...
                    //this next line sets genJetPassesdRCut = 0 if the deltaR between jet and muon is not greater than 0.4
		...
		if (genJetPassesdR02Cut) genJet.passDR02 = true;
                ...
                //andrew -- there is no dR cut on genJets being added to the genJetsNoDRCut collection
                //but genJet already has information assigned to it whether it passed the requirement or not
                //so one could make a cut on passDR04 on the genJetsNoDRCut collection later, as done below
		if (genJet.pt >= 10 && fabs(genJet.eta) <= 4.7){
		    genJetsNoDRCut.push_back(genJet);
                    npassesBasicGenJetCuts++;
		}
            }
	...
	double genMindRjmu(99999), genMindRj04Pt100mu(99999); 
        //double genMindRj1mu(99999); 
        //double genMindRj2mu(99999); 
	...
	int genIndClosestDR02Jet(-1), genIndEvtClosestDR04Jet(-1), genIndEvtClosestDR04DiJet(-1);
        ...
        if (hasGenInfo){
            ...
            for (unsigned short i(0); i < nGenJetsNoDRCut; i++){
                ...
                //andrew -- this line right below is making the analysis cuts on jet pt, jet abs rapidity, and dR between muon and jet
                //genJetsNoDRCut contains all of the jets that pass basic pT and rapidity cuts (these jets already carry passDR04 information)
                if (genJetsNoDRCut[i].pt >= jetPtCutMin && fabs(gjetr.Rapidity()) <= 0.1*jetEtaCutMax && genJetsNoDRCut[i].passDR04) {
                    tmpJets.push_back(genJetsNoDRCut[i]);
                    npassesAnalysisGenJetCuts++;
                }
		...
		if (genJetsNoDRCut[i].pt >= 100			&& fabs(gjetr.Rapidity()) <= 0.1*jetEtaCutMax && genJetsNoDRCut[i].passDR04) genJetsPt100DR04.push_back(genJetsNoDRCut[i]);
            ...
            //andrew -- and now it's here that we previously clear whatever was in genJets and then set it equal to the tmpJets collection (which is the part of the genJetsNoDRCut collection that passed the analysis cuts)
            genJets.clear();
            genJets = tmpJets; 
            tmpJets.clear(); 
            nGoodGenJets = genJets.size();
	    ...
	    nGenJetsPt100DR04 = genJetsPt100DR04.size();
            ...
            //need to double check that all the jets in genJets collection pass the requirements
            //genJets is the collection used to fill all the gen histograms below
            int passesGenJetPtMinCut(0);
            int passesGenJetAbsRapCut(0);
            int passesGenJetDR04Cut(0);
            for (unsigned short i(0); i < nGoodGenJets; i++){
                 TLorentzVector gjetvectemp;
                 gjetvectemp.SetPtEtaPhiE(genJets[i].pt, genJets[i].eta, genJets[i].phi, genJets[i].energy);
                 if (genJets[i].pt >= jetPtCutMin) passesGenJetPtMinCut++;
                 if (fabs(gjetvectemp.Rapidity()) <= 0.1*jetEtaCutMax) passesGenJetAbsRapCut++;
                 if (genJets[i].passDR04) passesGenJetDR04Cut++;
            }
            //andrew -- line below is requirement to have >=1 jets that pass the analysis requirements in the event 
            //EDIT: turn this off for now, should not affect anything
            //if (nGoodGenJets >= 1) {
                //mandate that every jet that will go to fill histograms pass these three requirements
                if ((passesGenJetPtMinCut == nGoodGenJets) && (passesGenJetAbsRapCut == nGoodGenJets) && (passesGenJetDR04Cut == nGoodGenJets)) {
                    passesGenJetCut = 1;
                    npassesGenJetCut++;
                }
            //}

            //cout << "nGoodGenJets: " << nGoodGenJets << ", passesGenJetPtMinCut: " << passesGenJetPtMinCut << ", passesGenJetAbsRapCut: " << passesGenJetAbsRapCut << ", passesGenJetDR04Cut: " << passesGenJetDR04Cut << endl;
	    ...
	    if (nGenJetsPt100DR04 >= 1) sort(genJetsPt100DR04.begin(), genJetsPt100DR04.end(), JetDescendingOrder);
            ...
            if (nGoodGenJets >= 1){
                ...
                //**************************************** end edit ************************************************************//
                //andrew -- don't think we need these next three lines since our jetPtCutMax currently set at 0 -- 5.4.18
                //if (jetPtCutMax > jetPtCutMin){
                //    passesGenJetCut = genJets[0].pt < jetPtCutMax;
                //    if (passesGenJetCut) npassesGenJetCut++;
                //}
            }
            if (nGoodGenJets >= 2){
        ...
        if (hasPartonInfo){
            ...
            if (fileName.find("PartonInfo") != string::npos && fileName.find("enriched") != string::npos &&
                ...
                if (!passesDPSPartonCut) npassesDPSPartonCut-- ;
            }
        }
        //=======================================================================================================//
...
//                }
                ...
                if (nGoodGenJets >= 1){
                    ...
                    //andrew
                    genLeadingJetPt_Zinc1jet_R21->Fill(genJets[0].pt, genWeight, mcEventWeight_);
                    genLepPtPlusLeadingJetPt_Zinc1jet_R21->Fill(genLep1.Pt()+genJets[0].pt, genWeight, mcEventWeight_);
                    genLepPtPlusHT_Zinc1jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_1_Zinc1jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_2_Zinc1jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_3_Zinc1jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_4_Zinc1jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                ...
                if (nGoodGenJets >= 2){
                    ...
                    //andrew
                    genLeadingJetPt_Zinc2jet_R21->Fill(genJets[0].pt, genWeight, mcEventWeight_);
                    genLeadingJetPt_Zinc2jet_R32->Fill(genJets[0].pt, genWeight, mcEventWeight_);
                    genHTover2_Zinc2jet_R32->Fill((genJets[0].pt + genJets[1].pt)/2. , genWeight, mcEventWeight_);
                    genLepPtPlusLeadingJetPt_Zinc2jet_R21->Fill(genLep1.Pt()+genJets[0].pt, genWeight, mcEventWeight_);
                    genLepPtPlusLeadingJetPt_Zinc2jet_R32->Fill(genLep1.Pt()+genJets[0].pt, genWeight, mcEventWeight_);
                    genLepPtPlusHT_Zinc2jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_Zinc2jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_1_Zinc2jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_1_Zinc2jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_2_Zinc2jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_2_Zinc2jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_3_Zinc2jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_3_Zinc2jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_4_Zinc2jet_R21->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_4_Zinc2jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_Zinc2jet_R32->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_1_Zinc2jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_2_Zinc2jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_3_Zinc2jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_4_Zinc2jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                ...
                if (nGoodGenJets >= 3){
                    ...
                    //andrew
                    genLeadingJetPt_Zinc3jet_R32->Fill(genJets[0].pt, genWeight, mcEventWeight_);
                    genLeadingJetPt_Zinc3jet_R43->Fill(genJets[0].pt, genWeight, mcEventWeight_);
                    genHTover2_Zinc3jet_R32->Fill((genJets[0].pt + genJets[1].pt)/2. , genWeight, mcEventWeight_);
                    genHTover2_Zinc3jet_R43->Fill((genJets[0].pt + genJets[1].pt)/2. , genWeight, mcEventWeight_);
                    genHTover3_Zinc3jet_R43->Fill((genJets[0].pt + genJets[1].pt + genJets[2].pt)/3. , genWeight, mcEventWeight_);
                    genLepPtPlusLeadingJetPt_Zinc3jet_R32->Fill(genLep1.Pt()+genJets[0].pt, genWeight, mcEventWeight_);
                    genLepPtPlusLeadingJetPt_Zinc3jet_R43->Fill(genLep1.Pt()+genJets[0].pt, genWeight, mcEventWeight_);
                    genLepPtPlusHT_Zinc3jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_Zinc3jet_R43->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_1_Zinc3jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_1_Zinc3jet_R43->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_2_Zinc3jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_2_Zinc3jet_R43->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_3_Zinc3jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_3_Zinc3jet_R43->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_4_Zinc3jet_R32->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_4_Zinc3jet_R43->Fill(genLep1.Pt() + genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_Zinc3jet_R32->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_Zinc3jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_1_Zinc3jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_1_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_2_Zinc3jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_2_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_3_Zinc3jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_3_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_4_Zinc3jet_R32->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_4_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_Zinc3jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);  
                    genLepPtPlusHTover3_1_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_2_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_3_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_4_Zinc3jet_R43->Fill(genLep1.Pt() + (genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                ...
                if (nGoodGenJets >= 4){
                    ...
                    //andrew
                    genLeadingJetPt_Zinc4jet_R43->Fill(genJets[0].pt, genWeight, mcEventWeight_);
                    genHTover2_Zinc4jet_R43->Fill((genJets[0].pt + genJets[1].pt)/2. , genWeight, mcEventWeight_);
                    genHTover3_Zinc4jet_R43->Fill((genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusLeadingJetPt_Zinc4jet_R43->Fill(genLep1.Pt()+genJets[0].pt, genWeight, mcEventWeight_);
                    genLepPtPlusHT_Zinc4jet_R43->Fill(genLep1.Pt()+genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_1_Zinc4jet_R43->Fill(genLep1.Pt()+genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_2_Zinc4jet_R43->Fill(genLep1.Pt()+ genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_3_Zinc4jet_R43->Fill(genLep1.Pt()+ genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHT_4_Zinc4jet_R43->Fill(genLep1.Pt()+ genJetsHT, genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_1_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_2_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_3_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover2_4_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_1_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_2_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_3_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
                    genLepPtPlusHTover3_4_Zinc4jet_R43->Fill(genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., genWeight, mcEventWeight_);
...
//            }
            ...
            if (nGoodJets >= 1){
                ...
                LepPtPlusHT_2_Zinc1jet_R21->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_3_Zinc1jet_R21->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_4_Zinc1jet_R21->Fill(lepton1.pt + jetsHT, weight);
            ...
            if (nGoodJets >= 2){
                ...
                LepPtPlusHT_2_Zinc2jet_R21->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_2_Zinc2jet_R32->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_2_Zinc2jet_R21->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_3_Zinc2jet_R21->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_3_Zinc2jet_R32->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_4_Zinc2jet_R21->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_4_Zinc2jet_R32->Fill(lepton1.pt + jetsHT, weight);
                ...
                LepPtPlusHTover2_2_Zinc2jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_3_Zinc2jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_4_Zinc2jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
...
//                } ///END (passesEWKJetPt) loop
            ...
            if (nGoodJets >= 3) {
                ...
                LepPtPlusHT_2_Zinc3jet_R32->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_2_Zinc3jet_R43->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_3_Zinc3jet_R32->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_3_Zinc3jet_R43->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_4_Zinc3jet_R32->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_4_Zinc3jet_R43->Fill(lepton1.pt + jetsHT, weight);
                ...
                LepPtPlusHTover2_2_Zinc3jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_2_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_3_Zinc3jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_3_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_4_Zinc3jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_4_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                ...
                LepPtPlusHTover3_2_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., weight);
                LepPtPlusHTover3_3_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., weight);
                LepPtPlusHTover3_4_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., weight);
            ...
            if (nGoodJets >= 4){
                ...
                LepPtPlusHT_2_Zinc4jet_R43->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_3_Zinc4jet_R43->Fill(lepton1.pt + jetsHT, weight);
                LepPtPlusHT_4_Zinc4jet_R43->Fill(lepton1.pt + jetsHT, weight);
                ...
                LepPtPlusHTover2_2_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_3_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                LepPtPlusHTover2_4_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., weight);
                ...
                LepPtPlusHTover3_2_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., weight);
                LepPtPlusHTover3_3_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., weight);
                LepPtPlusHTover3_4_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., weight);
        ...
        if (hasRecoInfo && hasGenInfo && passesGenLeptonCut && passesLeptonCut){
            ...
            if (nGoodGenJets >= 1 && nGoodJets >= 1){
                ...
                hresponseLepPtPlusHT_2_Zinc1jet_R21->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                hresponseLepPtPlusHT_3_Zinc1jet_R21->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                hresponseLepPtPlusHT_4_Zinc1jet_R21->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
            ...
            if (nGoodGenJets >= 2 && nGoodJets >= 2){
                ...
                hresponseLepPtPlusHT_2_Zinc2jet_R21->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                hresponseLepPtPlusHT_2_Zinc2jet_R32->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                hresponseLepPtPlusHT_3_Zinc2jet_R21->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                hresponseLepPtPlusHT_3_Zinc2jet_R32->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                hresponseLepPtPlusHT_4_Zinc2jet_R21->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                hresponseLepPtPlusHT_4_Zinc2jet_R32->Fill(lepton1.pt + jetsHT, genLep1.Pt() + genJetsHT, weight);
                ...
                hresponseLepPtPlusHTover2_2_Zinc2jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_3_Zinc2jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_4_Zinc2jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
            ...
            if (nGoodGenJets >= 3 && nGoodJets >= 3){
                ...
                hresponseLepPtPlusHT_2_Zinc3jet_R32->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                hresponseLepPtPlusHT_2_Zinc3jet_R43->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                hresponseLepPtPlusHT_3_Zinc3jet_R32->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                hresponseLepPtPlusHT_3_Zinc3jet_R43->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                hresponseLepPtPlusHT_4_Zinc3jet_R32->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                hresponseLepPtPlusHT_4_Zinc3jet_R43->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                ...
                hresponseLepPtPlusHTover2_2_Zinc3jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_2_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_3_Zinc3jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_3_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_4_Zinc3jet_R32->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_4_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                ...
                hresponseLepPtPlusHTover3_2_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt+jets[2].pt)/3., genLep1.Pt()+(genJets[0].pt + genJets[1].pt+genJets[2].pt)/3., weight);
                hresponseLepPtPlusHTover3_3_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt+jets[2].pt)/3., genLep1.Pt()+(genJets[0].pt + genJets[1].pt+genJets[2].pt)/3., weight);
                hresponseLepPtPlusHTover3_4_Zinc3jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt+jets[2].pt)/3., genLep1.Pt()+(genJets[0].pt + genJets[1].pt+genJets[2].pt)/3., weight);
            ...
            if (nGoodGenJets >= 4 && nGoodJets >= 4){
                ...
                hresponseLepPtPlusHT_2_Zinc4jet_R43->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                hresponseLepPtPlusHT_3_Zinc4jet_R43->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                hresponseLepPtPlusHT_4_Zinc4jet_R43->Fill(lepton1.pt+jetsHT, genLep1.Pt()+genJetsHT, weight);
                ...
                hresponseLepPtPlusHTover2_2_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_3_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                hresponseLepPtPlusHTover2_4_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt)/2., genLep1.Pt()+(genJets[0].pt + genJets[1].pt)/2., weight);
                ...
                hresponseLepPtPlusHTover3_2_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., weight);
                hresponseLepPtPlusHTover3_3_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., weight);
                hresponseLepPtPlusHTover3_4_Zinc4jet_R43->Fill(lepton1.pt + (jets[0].pt+jets[1].pt + jets[2].pt)/3., genLep1.Pt()+(genJets[0].pt + genJets[1].pt + genJets[2].pt)/3., weight);
    ...
    std::cout << "\nGen Lepton Status --- " << std::endl;
    std::cout << "#events considered/looped over: " << nEvents << std::endl;
    std::cout << "#gen leptons in all events that have relevant PDG ID: " << npassesGenLepSelector << std::endl;
    std::cout << "#gen leptons in all events that are GLepBarePrompt: " << npassesGenLepBarePrompt << std::endl;
    std::cout << "#prompt gen neutrinos in all events: " << numGenPromptNeutrinos << std::endl;
    std::cout << "#prompt gen electrons in all events: " << numGenPromptElectrons << std::endl;
    std::cout << "#prompt gen muons in all events: " << numGenPromptMuons << std::endl;
    std::cout << "#gen dressed charged leptons or neutrinos in all events that pass their resp. analysis cuts: " << npassesGenLeptonAnalysisCuts << std::endl;

    std::cout << "\nGen Particle Info --- " << std::endl;
    std::cout << "Gen Lepton Info: " << std::endl;
    std::cout << "--> #events w/ at least two gen leptons that have passed resp. analysis cuts: " << numHasAtLeastTwoGenLeptons << std::endl;
    std::cout << "--> #events that pass MTCut: " << npassesMTCut << std::endl;
    std::cout << "^^^these events have required the two highest pT leptons to exactly one muon and one neutrino that have passed their resp. analysis requirements, and that their MT has passed the analysis cut as well" << std::endl;
    std::cout << "--> #events with selected neutrino that pass METcut: " << npassesMETcut << std::endl;
    std::cout << "--> #events w/ passesGenLeptonCut: " << npassesGenLeptonCut << std::endl;
    std::cout << "^^^this should be the final event selection cut on leptons, MT, MET" << std::endl;
    std::cout << "DPS Parton Info: " << std::endl;
    std::cout << "--> #events w/ passesDPSPartonCut: " << npassesDPSPartonCut << std::endl;
    std::cout << "Gen Jet Info: " << std::endl;
    std::cout << "--> #gen jets in all events that very basic pT and eta cuts: " << npassesBasicGenJetCuts << std::endl;
    std::cout << "--> #gen jets in all events that pass analysis cuts (pT, rapidity, dR(mu,j)): " << npassesAnalysisGenJetCuts << std::endl;
    std::cout << "--> #events w/ all jets passing analysis requirements: " << npassesGenJetCut << std::endl;
    std::cout << "#events that are considered in filling gen histograms: " << GENnEventsIncl0Jets << std::endl;
    std::cout << "#events considered to fill gen histos with >= 1 GoodGenJets: " << GENnEventsIncl1Jets << std::endl; 	
...
void ZJetsAndDPS::getMcNorm_amcNLO(string inFile_Name, string storageElement){
	...
	while (getline(ifs, line)){
		...
		countFiles++;
		cout <<  "Line in List txt file: " << line << endl;
		
		TFile *f;

                if (fileName.find("gen_amcNLO") != string::npos) {
                    //for original GEN file
		    f = new TFile(("../DataW_AMCNLO/" + line).c_str());
		}
		else {
	   	    //for alpha-s
  	            f = new TFile((storageElement + line).c_str());
		}
